...
public class TargetZoneBullseye : MonoBehaviour
{
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("stone"))
        {
            Debug.Log("Bullseye Collision");
            other.gameObject.GetComponent<CurlingStone>().Parameters.Status.IsInScoringZone = true;
            other.gameObject.GetComponent<CurlingStone>().Parameters.Status.IsInScoringZoneBullseye = true;
        }
    }
    ...
    private void OnTriggerExit(Collider other)
    {
        if (other.CompareTag("stone"))
        {
            ...
            other.gameObject.GetComponent<CurlingStone>().Parameters.Status.IsInScoringZoneBullseye = false;
        }
    }
}